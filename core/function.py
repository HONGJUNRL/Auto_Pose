...
def validate_with_multi_scale(config, val_loader, val_dataset, model, criterion, output_dir,
             tb_log_dir, writer_dict=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    acc = AverageMeter()
    ...
    height = config.MODEL.HEATMAP_SIZE[1]
    ...
    with torch.no_grad():
        ...
        for i, (input, target, target_weight, meta) in enumerate(val_loader):
            ...
            multi_avg = torch.zeros((input[0].size(0), num_joints, height, width), dtype=torch.float32).cuda()
            for image in input:
                #print(image.size())
                output = model_input(image)
                #print(output.size())
                output = F.interpolate(output, size=(height,width), mode='bilinear', align_corners=True)
                multi_avg = multi_avg + output
            multi_avg = multi_avg / len(input)
            ...
            # measure accuracy and record loss
            losses.update(loss.item(), num_images)
            _, avg_acc, cnt, pred = accuracy(multi_avg.cpu().numpy(),
                                             target.cpu().numpy())
            ...
            preds, maxvals = get_final_preds(
                config, multi_avg.clone().cpu().numpy(), c, s)
            ...
            if i % config.PRINT_FREQ == 0:
                ...
                save_debug_images(config, input, meta, target, pred*4, multi_avg,
                                  prefix)
...
def padRightDownCorner(img, stride=32, padValue=128):
